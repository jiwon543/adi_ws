...
> **2026-1 스마트모빌리티종합설계**
> LIMO 로봇 기반 자율주행 파이프라인 개발 워크스페이스

---

## 개요

LIMO 로봇(AgileX)을 플랫폼으로 사용하는 자율주행 종합설계 프로젝트입니다.
카메라 · LiDAR 센서 인식부터 판단, 제어까지의 전체 파이프라인을 ROS Noetic 기반으로 구성합니다.
Docker 환경을 지원하며, 경량 Vision-Language Model(Moondream2)을 이용한 장면 이해 기능도 포함합니다.
...
adi_ws/
├── src/                    ← 주 개발 패키지
│   ├── Perception/         ← 인식 (카메라 차선 감지, LiDAR)
│   ├── Decision/           ← 판단 (VLM 판단 기반 미션 상태머신 등)
│   ├── Control/            ← 제어 (전역/지역 경로 제어기)
│   ├── Moondream/          ← VLM 기반 장면 이해 (Moondream2)
│   └── bringup/            ← 센서 브링업 (LIMO base, YDLiDAR, Astra)
