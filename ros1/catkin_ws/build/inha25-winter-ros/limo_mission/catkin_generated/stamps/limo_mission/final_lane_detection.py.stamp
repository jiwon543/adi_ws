...
class LaneDetectionNode:
    ...
    def state_callback(self, msg):
        ...
        - LANE_FOLLOWING: 차선 주행 활성화 (기본 속도)
        - ROTARY: 로터리 주행 활성화 (저속)
        - STOP: 정지 (다른 미션 노드에서 발행)
        """
        self.current_state = msg.data
        state_upper = self.current_state.upper()
        
        # LANE_FOLLOWING 또는 ROTARY면 활성화
        self.enabled = state_upper in ["LANE_FOLLOWING", "ROTARY"]
        
        # 상태별 속도 조정
        if state_upper == "ROTARY":
            self.base_speed = 0.3
        else:
            self.base_speed = self.default_base_speed  # 기본 속도로 복원
            
        rospy.loginfo_throttle(1.0, f"Lane State: {self.current_state}, Enabled: {self.enabled}, Speed: {self.base_speed}")
