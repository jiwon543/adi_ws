...
수정사항:
  - ROI 크롭 적용 (상단 불필요 영역 제거 → hallucination 감소)
  - 프롬프트 개선: open-ended WH 질문으로 시각 어텐션 유지
  - parse_mission 로직 강화: 우선순위 명확화 + edge case 처리
  - shutdown hook 추가
"""
...
# 트랙 배치 순서에 따른 우선순위 파싱 (먼저 매칭된 룰 우선)
...
def parse_mission(raw_text):
    """우선순위 기반 키워드 매칭. fallback = normal_drive"""
    text = raw_text.lower().strip()

    # 빈 출력 또는 "nothing" 계열 → normal
    if not text or text in ("nothing", "none", "n/a", "no"):
        return "normal_drive"

    # "clear road", "empty road", "just road" 등 명시적 클리어
...
def preprocess(msg):
    ...
    h = cv_img.shape[0]
    roi_top = g_cfg.get("roi_top_ratio", 0.4)
...
def main():
    ...
    rospy.loginfo("[VLM] Ready | interval=%.1fs | roi_top=%.0f%%",
                  ...
                  g_cfg.get("roi_top_ratio",  0.4) * 100)
    rospy.spin()
